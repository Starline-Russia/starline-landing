...
test("v3 production HTML preserves the landing contract", async () => {
  ...
  assert.equal(cascadeTriggers.length, 6);
  assert.ok(cascadeTriggers.every((trigger) => /\btype="button"/.test(trigger)));
  assert.ok(cascadeTriggers.every((trigger) => /\baria-label="Открыть/.test(trigger)));
  assert.equal(cascadeImages.length, 6);
  assert.doesNotMatch(html, /palitra-cascade-screen-overlay/);
